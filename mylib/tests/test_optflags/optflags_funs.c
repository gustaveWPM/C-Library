...
{
    char *argv[] = {
        "./a.out", "test", "10", "-dummy", "-dunno", "25", NULL
    };
...
{
    char *argv[] = {"./a.out", "-dummy", "-dunno", "test", "test", "10", "-lulz", NULL};
    int argc = (int) my_two_dims_array_len((void **) argv);
    optflags_t *opt = my_optflags_builder(argc, argv, true, true);
...
{
    char *argv[] = {
        "./a.out", "-dummy", "-dunno", "test", "test", "10", "-lulz", NULL
    };
    int argc = (int) my_two_dims_array_len((void **) argv);
    optflags_t *opt = my_optflags_builder(argc, argv, false, true);
...
{
    char *argv[] = {
        "./a.out", "--dump", "10", "-a", "coton",
        "--dump", "10", "-a", "coton", "-a", "coton",
        "--dump", "10", NULL
    };
    int argc = (int) my_two_dims_array_len((void **) argv);
    optflags_t *opt = my_optflags_builder(argc, argv, false, false);
...
Test(optflags_test, incompatible_flag_verb_test, .init = my_criterion_listener)
{
    ...
    cr_assert(!opt);
    cr_assert_stderr_eq_str("[ERROR] INCOMPATIBLE FLAGS: --dump, -a\n");
}
...
Test(optflags_test, advanced_test, .init = my_criterion_listener)
{
    ...
    my_optflags_destroy(&opt);
    cr_assert_stderr_eq_str(
        "[ERROR] THIS FLAG SHOULD BE UNIQ: --dump\n"
    );
}
...
Test(optflags_test, req_value_test, .init = my_criterion_listener)
{
    ...
    cr_assert_stderr_eq_str(
        ...
        "[WARNING] UNKNOWN FLAG: -p\n"
        "[WARNING] UNKNOWN FLAG: -j\n"
        "[WARNING] UNKNOWN FLAG: -k\n"
        "[ERROR] THIS FLAG REQUIRES A VALUE: -j\n"
        "[ERROR] THIS FLAG REQUIRES A VALUE: -k\n"
    );
}
